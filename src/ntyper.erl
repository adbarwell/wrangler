...
run(ArgStr) ->
  %% io:format("ArgStr: ~p~n", [ArgStr]),
  %% io:format("string:tokens(ArgStr): ~p~n", [string:tokens(ArgStr, " ")]),

  {Args, Analysis} = process_cl_args(string:tokens(ArgStr, " ")),
  %% io:format("Args: ~p\n", [Args]),
  %% io:format("Analysis: ~p\n", [Analysis]),
  ...
  TypeInfo = get_type_info_silent(Analysis4),
  dialyzer_timing:stop(Timer),
  formatTypeInfo(TypeInfo).
  %% io:format("\nTyper analysis finished\n").
  %% erlang:halt(0).

-spec rshow(string()) -> any().
...
get_type_info_silent(#analysis{callgraph = CallGraph,
			    trust_plt = TrustPLT,
			    codeserver = CodeServer} = Analysis) ->
  StrippedCallGraph = remove_external_silent(CallGraph, TrustPLT),
  try
    NewPlt = dialyzer_succ_typings:analyze_callgraph(StrippedCallGraph,
						     TrustPLT, CodeServer),
    Analysis#analysis{callgraph = StrippedCallGraph, trust_plt = NewPlt}
...
remove_external_silent(CallGraph, PLT) ->
  {StrippedCG0, Ext} = dialyzer_callgraph:remove_external(CallGraph),
  case get_external(Ext, PLT) of
    [] -> ok;
    _Externals ->
...
%% -spec formatTypeInfo(analysis()) -> [{atom(), integer(), string()}].
-spec formatTypeInfo(analysis()) -> [{atom(), integer(), 
				      {{args, [atom()]}, {ret, [atom()]}}}].

formatTypeInfo(#analysis{mode = Mode, fms = _Files} = Analysis) ->
  case Mode of
    ?SHOW ->
      present(Analysis);
    ?SHOW_EXPORTED ->
...
-spec present(analysis()) -> [{atom(), integer(), 
				      {{args, [atom()]}, {ret, [atom()]}}}].

present(Analysis) ->
  foreachfms(Analysis#analysis.fms, Analysis, []).

foreachfms([], _Analysis, Acc) ->
  ?print(Acc),
  Acc;
...
get_type_string(F, A, Info, Mode) ->
  ...
  case Info#info.edoc of
	...
	{file, {contract, _}} -> "";
	_ ->
	  Prefix = lists:concat([erl_types:atom_to_string(F)]),
...
-spec present_type_info(string(), info()) -> [{atom(), integer(), 
				      {{args, [atom()]}, {ret, [atom()]}}}].

present_type_info(_File, Info) ->
  lists:reverse(foreach(Info#info.functions, Info, [])).
...
get_type_tuple(F, A, Info, _Mode) -> 
  Type = get_type_info({F, A}, Info#info.types),
  ?print(Type),
  case Type of
    {contract, C} ->
      ?print(F),
      Args = dialyzer_contracts:get_contract_args(C),
      Return = dialyzer_contracts:get_contract_return(C),
      ?print(Args),
      ?print(Return),
...
-spec process_cl_args(any()) -> {args(), analysis()}.
...
analyze_args(ArgList, Args, Analysis) ->
  %% io:format("ArgList: ~p~n", [ArgList]),

  {Result, Rest} = cl(ArgList),
  %% io:format("cl(ArgList): ~p~n", [cl(ArgList)]),
...
%%--------------------------------------------------------------------
%% Processing of Erlang options and arguments.
%%--------------------------------------------------------------------

%% process_erl_args(ArgList) ->
%%   {Args, Analysis} = analyze_erl_args(ArgList, #args{}, #analysis{}),
...
%% analyze_erl_args([], Args, Analysis) ->
%%   {Args, Analysis};
%% analyze_erl_args(ArgList, Args, Analysis) ->
%%   %% io:format("ArgList: ~p~n", [ArgList]),

%%   {Result, Rest} = erl(ArgList),
%%   %% io:format("Result: ~p~n", [Result]),
%%   %% io:format("Rest: ~p~n~n", [Rest]),

%%   {NewArgs, NewAnalysis} = analyze_result(Result, Args, Analysis),
%%   analyze_erl_args(Rest, NewArgs, NewAnalysis).
